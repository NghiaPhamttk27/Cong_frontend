...
import { getLatesFileFull } from "@/api/data";
...
// --- Component ---
export default function LatestFullPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const res = await getLatesFileFull();
        setData(res);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <>
      ...
      >
        ...
        <span style={{ fontWeight: "500" }}>Danh sách file mới nhất</span>
      </div>
      <div style={pageContainerStyle}>
        <div style={headerStyle}>
          <h2 style={headerTitleStyle}>Danh sách file mới nhất</h2>
        </div>
